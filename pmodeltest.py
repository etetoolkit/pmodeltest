...
def main():
    ...
    job_list = run_jobs(job_list, nprocs=opts.nprocs)
    ...
    if opts.outtrees:
        tree = re_run_best(
            ord_aic[0], job_list[ord_aic[0]]["cmd"], opts.algt, verbose=opts.verb
        )
...
def get_job_list(
    algt, wanted_models, speed=True, verbose=False, protein=False, sequential=False
):
    """
...
def run_jobs(job_list, nprocs=1):
    """
    run jobs, parallelizing in given number of CPUs
    """

    phyml_version = Popen(
        "{} --version".format(PHYML), shell=True, stdout=PIPE
    ).communicate()
    version = findall(b".*PhyML version ([0-9.]+).*", phyml_version[0])[0]

    global FILE_EXT

    if version.startswith(b"3.3"):
        FILE_EXT = "_phyml_stats.txt_%s"
    else:
        FILE_EXT = "_phyml_stats_%s.txt"

    p = Pool(int(nprocs))
    data = p.map_async(
        launch_job, [(jname, jdata) for jname, jdata in job_list.items()]
    )
    for jname, out, err in data.get():
        job_list[jname]["out"] = out
        job_list[jname]["err"] = err
        if (err and "help / error" not in err) or "error" in out.lower():
...
def parse_jobs(job_list, algt):
    ...
    try:
        for job in job_list:
            ...
            job_list[job]["tree"] = get_tree(
                algt + FILE_EXT.replace("stats", "tree") % job
            )
    except IOError:
        fname = algt + FILE_EXT % job
        raise Exception(
            "ERROR (parse_job): no outfile found: %s" % fname, file=sys.stderr
        )
    return job_list
...
def re_run(job_list, algt, cutoff=0.95, nprocs=1, verbose=False):
    """
    ...
    if verbose:
        ...
        table += "\n    ".join(job_list.keys()) + "\n"
        print(table)
    job_list = run_jobs(job_list, nprocs=nprocs)
...
def re_run_best(better, cmd, algt, verbose=True):
    ...
    (out, err) = Popen(
        ...
        stdout=PIPE,
        shell=True,
        stdin=Popen(["echo", "end"], stdout=PIPE).stdout,
    ).communicate()

    if err or "Err: " in str(out):
        exit("ERROR: problem at last run of phyml: " + out)
    tree = get_tree(algt + FILE_EXT.replace("stats", "tree") % better)
    if verbose:
        print("\n Corresponding estimations of rates/frequencies:\n")
        stats = parse_stats(algt + FILE_EXT % better)
...
def clean_all(job_list, algt):
    ...
    for job in job_list:
        Popen(
            "rm -f %s_%s.txt" % (algt, FILE_EXT.replace("stats", "tree") % job),
            shell=True,
        ).communicate()
        Popen("rm -f %s_%s" % (algt, FILE_EXT % (job)), shell=True).communicate()
...
def parse_stats(path):
    ...
    for line in open(path, "r", errors="ignore"):
        if " Log-likelihood:" in line:
            ...
            dic["prop. of invariant"] = float(
                sub(".*([0-9]+\.[0-9]+).*", "\\1", line.strip())
            )
        elif line.startswith("  - Gamma shape parameter:"):
            dic["gamma shape"] = float(sub(".*([0-9]+\.[0-9]+).*", "\\1", line.strip()))
...
def get_options():
    ...
    parser = ArgumentParser(
        ...
        AA models available are: %s"""
        % (",".join(model_list["dna"]), ",".join(model_list["aa"]))
    )
    ...
    parser.add_argument(
        "-o", dest="outfile", type=str, help="name of outfile tree (newick format)"
    )
    parser.add_argument(
        "--outtable", dest="outtable", type=str, help="name of output table file"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="phyml",
        help="[%(default)s]path to phyml binary",
    )
    parser.add_argument(
        "--support",
        action="store_true",
    ...
    parser.add_argument(
        ...
        dest="medium",
        default=False,
        help="""[%(default)s] Same as fast, but reruns models with
...
FREQS = {
    "nt": {"": ["-f", "0.25,0.25,0.25,0.25"], "+F": ["-f", "m"]},
    "aa": {"": ["-f", "m"], "+F": ["-f", "e"]},
}
